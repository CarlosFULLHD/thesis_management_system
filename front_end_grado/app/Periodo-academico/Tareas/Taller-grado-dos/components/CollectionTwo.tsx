...
import {
  ...
  useTaskHasDate,
} from "@/app/Periodo-academico/providers/TaskHasDateProvider";
...
const CollectionTwo = () => {
  ...
  const { taskHasDateList, loadTaskHasDateFromDB, loadTaskHasDateList } =
    useTaskHasDate();
  // Importing state from academicperiod provider
  const { mainAcademicPeriod, loadAcademicPeriodByItsIdFromDB } =
    useAcademicPeriod();
  ...
  const removeTaskAsDate = (idTask: number) => {
    setTaskWithDateList((prevIds) =>
      prevIds.filter((prevIds) => prevIds.taskIdTask.idTask !== idTask)
    );
  };
  ...
  const setInitialOrder = () => {
    ...
    const idToOrder = new Map(
      selectedTaskIds.map((id, index) => [id, index + 1])
    );
  ...
  const handlePublicationDateUpdate = (
    idTask: number,
    ...
    setTaskWithDateList((currentTasks: TaskHasDateInterface[]) => {
      const updatedTasks = currentTasks.map((task: TaskHasDateInterface) =>
        task.taskIdTask.idTask === idTask
          ? { ...task, publicationDate: newPublicationDate }
          : task
      );
      return updatedTasks;
    });
      ...
      <>
        <div>
          <h1 className="ttext-2xl md:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-teal-400">
            Programación tareas Semestre {mainAcademicPeriod.semester}
          </h1>
                ...
                <div className="grid grid-cols-6 md:grid-cols-12 gap-6 md:gap-4 items-center justify-center">
                  <div className="relative col-span-6 md:col-span-4 flex flex-col items-center justify-center space-y-2">
                    ...
                    <div className="flex flex-col items-start w-full">
                      ...
                      <Chip color="warning">
                        <Checkbox isSelected={item.isUrl == 1} color="success">
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex flex-col gap-0">
                        <h3 className="font-semibold text-foreground/90">
                          Título: {item.taskIdTask.titleTask}
                        </h3>
                        <p className="text-small text-foreground/80">
                          <b>Descripción:</b> {item.taskIdTask.task}
                        </p>
                      </div>
                    </div>
                    <Divider />
