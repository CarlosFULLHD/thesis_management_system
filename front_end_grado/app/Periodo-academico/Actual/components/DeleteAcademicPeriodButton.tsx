...
const DeleteAcademicPeriodButton = ({ idAcad }: DeleteAcademicPeriodButtonProps) => {
    // Importing data and method from provider
    const { mainAcademicPeriod, deleteMainAcademicPeriod } = useAcademicPeriod();

    const academicItem: AcademicPeriodItem = mainAcademicPeriod;
    ...
    const deleteResource = async (id: number): Promise<void> => {
        ...
        try {
            ...
            if (response.status >= 200 && response.status < 300) {
                deleteMainAcademicPeriod()// Updating provider
...
export default DeleteAcademicPeriodButton;