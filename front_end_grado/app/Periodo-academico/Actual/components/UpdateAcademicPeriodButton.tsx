...
const UpdateAcademicPeriodButton = ({
  idAcad,
}: UpdateAcademicPeriodButtonProps) => {
  // Importing provider
  ...
  const [initDate, setInitDate] = useState<string>(
    localAcademicPeriod.initDate
  );
  const [deadLineDate, setDeadLineDate] = useState<string>(
    localAcademicPeriod.endDate
  );
  const [accountUntil, setAccountUntil] = useState<string>(
    localAcademicPeriod.accountUntil
  );
  ...
  const updateResource = async (): Promise<void> => {
    ...
    try {
      if (
        initDate != localAcademicPeriod.initDate ||
        deadLineDate != localAcademicPeriod.endDate ||
        accountUntil != localAcademicPeriod.accountUntil
      ) {
  ...
  return (
    <div>
      <Tooltip color="primary" content="Modificar periodo">
        <Button key="blur" color="primary" variant="ghost" onPress={onOpen}>
          <FaEdit />
        </Button>
      </Tooltip>
      <Modal
      ...
      >
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1 md:text-2xl">
                ¿Modificar fechas del semestre {mainAcademicPeriod.semester}?
              </ModalHeader>
                ...
                <div className="flex flex-col gap-4">
                  <label>
                    ...
                    <DateTimePickerHtml
                      ...
                      dateValue={initDate}
                      bgColorClass="bg-custom-green"
                      fontColorClass="text-custom-green-font"
                    />
                  </label>
                  <label>
                    ...
                    <DateTimePickerHtml
                      ...
                      bgColorClass="bg-custom-red"
                      fontColorClass="text-custom-red-font"
                    />
                  </label>
                  <label>
                    ...
                    <DateTimePickerHtml
                      ...
                      dateValue={accountUntil}
                      bgColorClass="bg-custom-blue"
                      fontColorClass="text-custom-blue-font"
                    />
                  </label>
                </div>
              </ModalBody>
