...
const LoginForm = () => {
    ...
    return (
        <div>
            <form onSubmit={checkFormAndCaptcha} style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxWidth: '300px', margin: 'auto' }}>
                <div>
                    <Input fullWidth type="text" variant="bordered" label="Nombre de cuenta" value={account} onChange={(event) => setAccount(event.target.value)} required />
                </div>
                <div>
                    <Input fullWidth type="password" variant="bordered" label="Contraseña" value={password} onChange={(event) => setPassword(event.target.value)} required />
                </div>
