...
import { Avatar, Button, Card, CardBody, CardFooter, CardHeader, CircularProgress, Divider } from "@nextui-org/react";
...
const GradeProfileLecturerCollection = () => {
    ...
    const { gradeProfileLecturerList, loadGradeProfileLecturerList } = useGradeProfileLecturerCollection();
                            ...
                            <Card className="max-w-[500px]" key={item.gradeProfile.idGradePro}>
                                    ...
                                    <div className="flex gap-5">
                                        ...
                                        <div className="flex flex-col gap-1 items-start justify-center">
                                            <h4 className="text-small font-semibold leading-none text-default-600">
                                                {item.gradeProfile.roleHasPerson?.usersIdUsers.personIdPerson.name ?? ''} {item.gradeProfile.roleHasPerson?.usersIdUsers.personIdPerson.fatherLastName ?? ''}
                                            </h4>
                                            <h5 className="text-small tracking-tight text-default-400">
                                                {item.gradeProfile.roleHasPerson?.usersIdUsers.personIdPerson.email ?? ''}
                                            </h5>
                                        </div>
                                    </div>
                                    <Button
                                ...
                                <CardBody >
                                    ...
                                    <div className="grid grid-cols-3 gap-2 border border-gray-300 rounded p-4">
                                            <TutorButton isDisabled={item.tutor == null ? false : true} idGradePro={item.gradeProfile.idGradePro}/>
                                        <div className="col-span-2 flex flex-col justify-center items-start">
                                            <h1 className="text-lg font-bold mb-2">
                                                Tutor
                                            </h1>
                                            <p className="text-sm">
                                                {item.tutor == null ? "SIN ASIGNAR" : `${item.tutor.roleHasPersonIdRolePer?.usersIdUsers.personIdPerson.name ?? ''} ${item.tutor.roleHasPersonIdRolePer?.usersIdUsers.personIdPerson.fatherLastName ?? ''}`}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    {/* LECTURER */}
                                    <div className="grid grid-cols-3 gap-2 border border-gray-300 rounded p-4">
                                            <LecturerButton isDisabled={item.lecturer == null ? false : true} idGradePro={item.gradeProfile.idGradePro} />
                                        <div className="col-span-2 flex flex-col justify-center items-start">
                                            <h1 className="text-lg font-bold mb-2">
                                                Relator
                                            </h1>
                                            <p className="text-sm">
                                                {item.lecturer == null ? "SIN ASIGNAR" : `${item.lecturer.roleHasPersonIdRolePer?.usersIdUsers.personIdPerson.name ?? ''} ${item.lecturer.roleHasPersonIdRolePer?.usersIdUsers.personIdPerson.fatherLastName ?? ''}`}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    {/* TITULO */}
                                    <div className="grid grid-cols-3 gap-2 border border-gray-300 rounded p-4">
                                        <TitleButton isDisabled={item.gradeProfile.title == "" ? false : true} idGradePro={item.gradeProfile.idGradePro} title={item.gradeProfile.title} />
                                        <div className="col-span-2 flex flex-col justify-center items-start">
                                            <h1 className="text-lg font-bold mb-2">
                                                Título
                                            </h1>
                                            <p className="text-sm">
                                                {item.gradeProfile.title == "" ? "SIN ASIGNAR" : `${item.gradeProfile.title}`}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-3 gap-2 border border-gray-300 rounded p-4">
                                        <GraduationButton isDisabled={item.gradeProfile.statusGraduationMode == -1 ? false : true} idGradePro={item.gradeProfile.idGradePro} />
                                        <div className="col-span-2 flex flex-col justify-center items-start">
                                            <h1 className="text-lg font-bold mb-2">
                                    ...
                                    <div className="grid grid-cols-3 gap-2 border border-gray-300 rounded p-4">
                                        ...
                                        <div className="col-span-2 flex flex-col justify-center items-start">
                                            ...
                                            <p className="text-sm">
                                                {item.gradeProfile.isGradeoneortwo == 1 ? "Taller de grado 1" : "Taller de grado 2"}
                                            </p>
                                        </div>
                                    </div>
                                </CardBody>
                                {
                                    item.gradeProfile.statusGraduationMode == -1 || item.lecturer == null || item.lecturer == null || item.gradeProfile.title == ""
                                        ? <>
